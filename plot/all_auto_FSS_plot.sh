...
echo 'Z-R #3: Meiyu'
echo 'Z-R #4: QPESUMs'
read -p 'Enter the Z-R #:' type_num

fr=60 #forecast frequency (minute)
...
#Change the directory of forecast data in the following '.sh' files.
case ${now_num} in
  "1")
...
#Change the type of verification data in the following '.sh' files.
case ${type_num} in
  "1")
    type_name='alldata'  
    zr_eq='R=0.0279Z^0.6619'
    ;;
  "2")
    type_name='typhoon'  
    zr_eq='R=0.0282Z^0.6624'
    ;;
  "3")
    type_name='Meiyu'  
    zr_eq='R=0.0244Z^0.6779'
    ;;
  "4")
    type_name='QPESUMs'  
    zr_eq='R=0.0.121257Z^0.60606'
    ;;
esac
...
do
  ...
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_echo_index.nc echofile/T1/.
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_acrain_index.nc acrainfile/T1/.
...
do
  ...
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_echo_index.nc echofile/T2/.
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_acrain_index.nc acrainfile/T2/.
  timestamp=${now:0:4}-${now:4:2}-${now:6:2}' '${now:8:2}:${now:10:2}:${now:12:2}
...
do
  ...
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_echo_index.nc echofile/T3/.
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_acrain_index.nc acrainfile/T3/.
  timestamp=${now:0:4}-${now:4:2}-${now:6:2}' '${now:8:2}:${now:10:2}:${now:12:2}
...
do
 ## 
  if [ ! -e ${case_name}/${now}/${now}_fss_echo_index.nc ]; then
  sed -i '15c Case_name="'${case_name}'"' echo_to_nc.ncl
  sed -i '16c fTime="'${now}'"' echo_to_nc.ncl
  ncl echo_to_nc.ncl
  mv ${case_name}_${now}*.png ${case_name}/.
  else
  echo 'Already Done'
  fi
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_echo_index.nc echofile/All/.
  ln -sf /home/miayao/maple_ncu/FSS/${case_name}/${now}/${now}_fss_acrain_index.nc acrainfile/All/.
  timestamp=${now:0:4}-${now:4:2}-${now:6:2}' '${now:8:2}:${now:10:2}:${now:12:2}
...
echo 'Verification data type: '$type_name
echo '=============================='
